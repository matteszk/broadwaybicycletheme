...
<div id="event_espresso_registration_form" class="event-display-boxes">
  <div class="event_espresso_form_wrapper event-data-display">
      ...
      <?php echo $is_active['status'] == 'DRAFT' ? ' - <span class="expired_event">Event is a Draft</span>' : ''; ?> </h2>
		...
		switch ($is_active['status']) {
			...
			case 'REGISTRATION_NOT_OPEN': //only show the event description.
				...
				<div class="event-registration-pending event-messages">
					...
					<p class="event_full">
						<strong>
							<?php _e('You will be able to register starting ', 'event_espresso');
              ...
              <h4>Event Details</h4>
					...
					if ($display_reg_form == 'Y') {
						...
						if ($display_price_dropdown == true) {
							?>
							<p class="event_prices mt0"><?php echo event_espresso_price_dropdown($event_id); //Show pricing in a dropdown or text ?></p>
							<?php
						} else {
						...
						<fieldset id="event-reg-form-groups" class="mt1 mb1">
							<?php
						...
						<input type="hidden" name="event_id" id="event_id-<?php echo $event_id; ?>" value="<?php echo $event_id; ?>">
						 ...
						 <p class="info">Please note - your online confirmation will not be valid without a payment. We accept Paypal and major credit cards. After completing this form, you will be taken to Paypal to complete your registration. Thanks! </p>
						<?php
						//Recaptcha portion
