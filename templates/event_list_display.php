...
<div id="event_data-<?php echo $event_id ?>" class="event_data <?php echo $css_class; ?> <?php echo $category_identifier; ?> event-data-display event-list-display event-display-boxes ui-widget mt2 col-xs-12 col-sm-6 col-lg-4">
...
<div class="event-data-display ui-widget-content ui-corner-bottom">
	...
	<div class="event-meta">
		...
		<p class="mt0" id="event_date-<?php echo $event_id ?>"><span class="section-title"><?php _e('', 'event_espresso'); ?></span>
			<?php echo  date("l\s\, F d", strtotime($start_date)) . '–' .  date("F d", strtotime($end_date)) ; ?>
			<?php //Add to calendar button
			echo apply_filters('filter_hook_espresso_display_ical', $all_meta);?>
		</p>
		<p class="mt0"><?php echo date('g:ia', strtotime($event->start_time)) . '–' . date('g:ia', strtotime($event->end_time)); ?></p>
		...
		if ($display_reg_form == 'Y') {
			...
			?>
				<p id="register_link-<?php echo $event_id ?>" class="register-link-footer">
					<a class="a_register_link ui-button ui-button-big ui-priority-primary ui-state-default ui-state-hover ui-state-focus ui-corner-all mt1 btn" id="a_register_link-<?php echo $event_id ?>" href="<?php echo $registration_url; ?>" title="<?php echo stripslashes_deep($event_name) ?>"><?php _e('Register', 'event_espresso'); ?></a>
	...
	?>
			<p id="register_link-<?php echo $event_id ?>" class="register-link-footer">
				<a class="a_register_link ui-button ui-button-big ui-priority-primary ui-state-default ui-state-hover ui-state-focus ui-corner-all mt1 btn" id="a_register_link-<?php echo $event_id ?>" href="<?php echo $registration_url; ?>" title="<?php echo stripslashes_deep($event_name) ?>"><?php _e('View Details', 'event_espresso'); ?></a> <?php echo isset($cart_link) && $externalURL == '' ? $cart_link : ''; ?>
			</p>
